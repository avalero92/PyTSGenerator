"""
download_hrvpp.py — Módulo de búsqueda y descarga de productos HR-VPP.

Producto: High-Resolution Vegetation Phenology and Productivity (HR-VPP)
Dataset:  EO:EEA:DAT:CLMS_HRVPP_VPP
...
class DownloadHRVPPModule(ReprojectMixin, CropMixin, DownloadBaseModule):
    """
    Módulo de descarga de productos HR-VPP.

    MRO: DownloadHRVPPModule → ReprojectMixin → CropMixin → DownloadBaseModule
    ...
    NAME            = "Download HR-VPP"
    ICON            = "🌿"
    DESCRIPTION     = (
        "Busca y descarga productos de Fenología de Alta Resolución (HR-VPP) "
        "del Copernicus Land Monitoring Service (CLMS) vía API HDA de WEkEO. "
        "Incluye indicadores de inicio/fin de estación, máximo de vegetación "
        "y productividad estacional. "
        "Admite delimitación por shapefile (BBox), reproyección de CRS opcional "
        "y recorte automático al área de interés."
    )
    DEFAULT_DATASET = "EO:EEA:DAT:CLMS_HRVPP_VPP"
    ...
    PRODUCT_FIELDS = [
        {
            ...
            "row":         0,
            "col":         1,
            "_help":       "SOSD · EOSD · MAXD · MINV · AMPL · LENGTH · LSLOPE · RSLOPE · SPROD · TPROD",
        },
        {
            "attr":        "var_product_group_id",
            "label":       "🗂️ Product Group ID",
            "placeholder": "s1",
            "query_key":   "productGroupId",
            "row":         0,
            "col":         2,
            "_help":       "s1 = primera estación  ·  s2 = segunda estación",
        },
    ]
    ...
    def on_product_downloaded(self, local_path: str) -> str:
        ...
        Pipeline post-descarga:
          1. Reproyección (si activada) — primero para que el recorte
             opere ya sobre el CRS final.
